...
MAPE <- function (data, lev = NULL, model = NULL) {
  ape <- abs(data$pred - data$obs) / map_dbl(data$pred, ~ max(abs(.x), 290000))
  out <- c(mean(ape))
  names(out) <- c("MAPE")
  out
} 
...
MAPE_gen <- function (pred, obs) {
  ape <- abs(pred - obs)/ map_dbl(pred, ~ max(abs(.x), 290000))
  out <- c(mean(ape))
  names(out) <- c("MAPE")
  out
} 
...
nrounds = 50, max_depth = 2, eta = 0.3, gamma = 0, colsample_bytree = 0.8, min_child_weight =
 1 and subsample = 1
